 ...
 *
 */
public class OnFinish2_LeadApproveAction extends JITActionBase implements Preparable {
	...
	public String approve() {
		try {
			...
			String hql="select count(*) as cnt from ItemApprove approve where approve.type='"+ItemApprove.TYPE_ONFINISH_LEAD_APPROVE+"' and approve.itemId='" + item.getId() + "' and approve.status='"+ItemApprove.STATUS_PENDING+"'";
			long cnt = (java.lang.Long) service.queryByHql(hql).get(0);
			
			if(cnt==0){//多个审批都通过才算审批通过
				item.setStatus(Item.STATUS_11_ON_FINISH_LEAD_APPROVED);
	...
	public String reject() {
		try {
			...
			item.setStatus(Item.STATUS_11_ON_FINISH_LEAD_REJECT);
			service.updateItem(item);
			itemApprove.setApproverId(u.getId());
			itemApprove.setApproverName(u.getName());
			itemApprove.setItemId(item.getId());
