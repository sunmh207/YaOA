...
function submitBid() {
	...
	frm.action="${root}/workflow/onbid1_biddoc!submitBid.do";  
 	frm.submit();
	}
}
</script>
...
<body>
<html:pagetitle title="项目招标->招标文件"/>

<s:actionerror />
<s:actionmessage />
<s:form id="bidForm" action="onbid1_biddoc!submitBid.do" method="post"  enctype="multipart/form-data">
...
<table width="99%" class="table" cellpadding="0" cellspacing="0" align="center">
    ...
    <tr>
     ...
     <tr>
        ...
        <td class="td_edit" colspan="3">	
        	<s:iterator value="recommendBidderList" status="st">
        		...
        		<a  href="${root}/workflow/onbid1_biddoc!recommendBidderInput.do?item.id=<s:property value="item.id"/>&recommendBidder.id=<s:property value="id"/>">编辑</a>&nbsp;
        		<a href="${root}/oa/download!downloadRecommendBidderDoc.do?recommendBidder.id=<s:property value="id"/>">下载附件</a>&nbsp;
        		<a  href="${root}/workflow/onbid1_biddoc!deleteRecommendBidder.do?item.id=<s:property value="item.id"/>&recommendBidder.id=<s:property value="id"/>">删除推荐单位</a>
        		<br>
        	</s:iterator> 
	        <div id="bidder">
     ...
     <tr>
        ...
        <a href="${root}/oa/download!downloadBidNegotiation.do?item.id=${item.id}&bid.itemId=${bi.itemId}">下载议标说明</a>	
        </td>
    </tr>
     <tr>
    ...
    <tr>
     ...
     <tr>
        <td height="24" align="right" class="td_lable">招标管理部门审批领导</td>
        <td class="td_edit" colspan="3">		
	...
	<tr>
      ...
      <td class="td_edit" colspan="3">	
      <input type="button" onclick="submitBid()"  value="提报招标管理部门审核">
