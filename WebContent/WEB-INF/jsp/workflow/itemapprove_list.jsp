...
<%-- <form id="searchForm"　action="${root}/workflow/itemapprove.do">
	...
	<s:iterator value="objectList" status="st">
		...
		<td align="center" class="td_header"><s:property value='#st.index+1'/> </td>
			...
			<s:if test="objectList[#st.index][1].status == \"PENDING\"">
				<a href="${root}/workflow/itemapproveinput!input.do?item.id=<s:property  value="objectList[#st.index][0].id"/>&itemApprove.id=<s:property  value="objectList[#st.index][1].id"/>">审批</a>&nbsp;
			</s:if>
