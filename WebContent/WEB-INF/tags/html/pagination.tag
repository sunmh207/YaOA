...
<div id='Pagination' style='float:right;'>记录总数:<strong>${pager.totalRecords}</strong>&nbsp;
	...
	项每页&nbsp;页码:<strong>${pager.currentPage}</strong>/<strong>${pager.totalPages}</strong>&nbsp;
	<c:choose>
		<c:when test="${pager.currentPage eq 1}">首页&nbsp;上一页</c:when>
		<c:otherwise>
			<a href="javascript:goPage(1)">首页</a>&nbsp;
			<a href="javascript:goPage(${pager.currentPage-1})">上一页</a>
		</c:otherwise>
	</c:choose>
	<c:choose>
		<c:when test="${pager.currentPage eq pager.totalPages}">下一页&nbsp;尾页</c:when>
		<c:otherwise>
			<a href="javascript:goPage(${pager.currentPage+1})">下一页</a>&nbsp;
			<a href="javascript:goPage(${pager.totalPages})">尾页</a>
		</c:otherwise>
	</c:choose>
	<input type='text' id='_PageID' value='${pager.currentPage}' name="currentPage"
		style='text-align:center;width:30px;height:13px;font-size:12px; height:20px;'>
