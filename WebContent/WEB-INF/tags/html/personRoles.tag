...
<%@ attribute name="personId" type="java.lang.String" required="true"%>
<%!
public String getRolesName(String personId){
	String names="";
	try{
	//Session session = DBtools.getExclusiveSession();
	UserRoleService urs=  new UserRoleService();
	java.util.List<Role> userRoles = urs.queryRoleByMemberId(personId);
...
%>
<%
	out.print(getRolesName(personId));
%>